...
render theScreen obj = do
  ...
  printAt theScreen (x, y) (Console.White, (show obj))

renderScene :: Handle -> Scene -> IO ()
renderScene theScreen theScene = do
  renderMap theScreen (map theScene)
  mapM_ (render theScreen) (objects theScene)
  mapM_ (render theScreen) (monsters theScene)
...
showControls theScreen (x, y) = do
  printAt theScreen (x, y) (Console.Magenta, "Controls: [i,j,k,l] = move, a = attack")
